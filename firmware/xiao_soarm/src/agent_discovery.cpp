...
private:
    ...
    uint8_t next_probe_host_ = kFirstHost;
    bool has_agent_ = false;
    bool unsupported_reported_ = false;
};

void RuntimeDiscovery::begin() {
...
bool RuntimeDiscovery::has_agent() const {
    return has_agent_;
}

const IPAddress& RuntimeDiscovery::agent_ip() const {
    return agent_ip_;
}

uint16_t RuntimeDiscovery::agent_port() const {
    return agent_port_;
}

bool RuntimeDiscovery::is_supported_subnet() const {
    return WiFi.subnetMask() == IPAddress(255, 255, 255, 0);
}

void RuntimeDiscovery::send_probe_if_due(uint32_t now_ms) {
...
Agent discover() {
    ...
    return Agent{discovery.agent_ip(), discovery.agent_port()};
}
