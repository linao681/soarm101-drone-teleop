...
| 7 | 六关节慢动作；速度限制、温度、电压正常且无拒绝 | 2026-09-08 | `1067dff` | linao | 舵机电源接通，机械支撑，空间清空，可立即断电 | 部分完成；143.76 s CSV 中 `response_mask=63`、拒绝 0、超时 0；操作员确认六关节方向正确且响应可以；本次未保存电压/温度读数，不能判定完整通过 | `logs/speed-profile/2026-09-08-powered-response.md`; `/home/linao/so101_lerobot/logs/teleop_20260908_193759.csv` |
| 8 | 停止桥接超过 500 ms；进入 `HOLDING_TIMEOUT`、保持姿态并近当前姿态重新握手 | 2026-09-08 | `68ad76b` | linao | 舵机电源接通，机械臂受支撑，随时可断电 | 通过；停止后进入 `HOLDING_TIMEOUT` 并保持姿态，重启生成新 session 且无跳动 | `logs/gate-08/2026-09-08-timeout-rehandshake.md` |
| 9 | 禁用并恢复 WiFi；生成新会话且不重放旧目标 | 2026-09-08 | `68ad76b` | linao | 舵机电源接通，机械臂受支撑，网络可控 | 通过；WiFi 中断约 3.229 秒后生成新 session，近当前姿态握手无跳动且未回放旧目标 | `logs/gate-09/2026-09-08-wifi-recovery.md`; `logs/teleop_20260908_184248.csv` |
